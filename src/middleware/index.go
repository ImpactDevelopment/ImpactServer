...
func EnforceHTTPS(code int) echo.MiddlewareFunc {
	return func(next echo.HandlerFunc) echo.HandlerFunc {
		return func(c echo.Context) error {
			...
			// it is http
			addr := c.Request().URL
			if addr.Path == "/releases.json" || ((strings.Contains(addr.Path, "/Impact-") || strings.HasPrefix(addr.Path, "/maven.refmap.json")) && (strings.HasSuffix(addr.Path, ".json") || strings.HasSuffix(addr.Path, ".json.asc"))) {
